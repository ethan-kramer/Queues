...
void DynamicQueue::insert(char x) {
	...
	if (empty()) {
		...
		rear->next = p;	// puts p at end 
	}

	rear = p;	// moves rear to point to p
	if (size != QUEUE_SIZE) {
		size++;
	}
}
...
int DynamicQueue::remove() {
	...
	DynamicNode* p = front;
	int temp = p->info;
	front = p->next;	// front points at next element
	delete p;
	if (front == nullptr)
		rear = nullptr;
...
void DynamicQueue::insertOrMove(char x) {
	...
	if (front == rear && front->info == x) {	// only one element and element already = x
		return;
	}
	...
	if (q->info == x) {	// if first element, move to rear
		front = front->next;
		delete q;
		insert(x);
	...
	while (p != nullptr) {
		...
		if (p->info == x) {//IN QUEUE
			...
			q->next = p->next;	// breaks link to p
			delete p;
			insert(x);
			std::cout << "Moving " << x << " to rear. ";
	...
	if (size != QUEUE_SIZE) {
		...
		if (size = QUEUE_SIZE) {
			//remove front element of the queue; then, insert new node with key in rear.
			p = front;
			front = front->next;
			delete p;
			insert(x);
